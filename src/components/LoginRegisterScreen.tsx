...
import { registerUserAccountToSupabase, loginUserAccountToSupabase, copyLocalStorageData, supabase } from '../lib/supabase';
...
export function LoginRegisterScreen({ onLogin, onBackToLanding, initialIsRegister = false }: LoginRegisterScreenProps) {
  ...
  const handleGoogleSubmit = async (e: React.FormEvent) => {
    ...
    if (!existingUser) {
      ...
      let finalId = newUserId;
      try {
        const supabaseId = await registerUserAccountToSupabase(existingUser);
        if (supabaseId) {
          finalId = supabaseId;
      ...
      const profileData: UserProfile = {
        ...
        avatar: 'preset-indigo'
      };
      window.localStorage.setItem(profileKey, JSON.stringify(profileData));
      onLogin(finalId);
    } else {
      let finalId = existingUser.id;
      try {
        const supabaseId = await loginUserAccountToSupabase(emailLower, 'google-oauth-secured');
        if (supabaseId) {
          finalId = supabaseId;
          if (existingUser.id !== supabaseId) {
            copyLocalStorageData(existingUser.id, supabaseId);
            existingUser.id = supabaseId;
            setUsers(users.map(u => u.email.toLowerCase().trim() === emailLower ? existingUser! : u));
          }
        }
      } catch (err) {
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-3xl max-w-sm w-full p-7 shadow-2xl space-y-5 relative">
            <button 
              ...
              className="absolute top-4 right-4 p-1.5 text-gray-400 hover:text-gray-600 cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-gray-900 dark:text-zinc-100">Google Authentication</h3>
                <p className="text-[10px] text-gray-500 dark:text-zinc-400">Sign in securely via Google</p>
              </div>
            </div>
            ...
            <form onSubmit={handleGoogleSubmit} className="space-y-3.5">
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-wider">Google Email</label>
                <input 
                  type="email" 
                  required 
                  value={googleEmail} 
              ...
              {/* Google Modal Custom Dropdown */}
              <div className="space-y-1.5 relative">
                <label className="block text-[10px] font-black text-gray-550 dark:text-zinc-400 uppercase tracking-wider">Profile Role</label>
                <button
                  type="button"
                  onClick={() => handleDropdownToggle('googleRole')}
                  className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl border border-gray-250 dark:border-zinc-700 bg-gray-50/40 dark:bg-zinc-950/20 text-gray-900 dark:text-zinc-100 focus:border-[#00A896] text-left text-sm"
                >
                  <span className="truncate">{googleRole}</span>
                  <ChevronDown className="w-4 h-4 text-gray-400" />
                </button>

                {isGoogleRoleDropdownOpen && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-xl z-40 max-h-40 overflow-y-auto py-1">
                    {['Personal Budgeter', 'Salaried Employee', 'Freelance Specialist', 'Business Owner', 'Platform Administrator'].map(r => (
                      <button
                        key={r}
                        type="button"
                        onClick={() => {
                        ...
                        className={`w-full flex items-center justify-between px-4 py-2.5 text-xs text-left hover:bg-gray-100 dark:hover:bg-zinc-800/80 ${
                          googleRole === r ? 'text-[#00A896] font-bold' : 'text-gray-700 dark:text-zinc-200'
                        }`}
                      >
                        <span>{r}</span>
                        {googleRole === r && <Check className="w-3.5 h-3.5 text-[#00A896]" />}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <button 
                type="submit" 
                className="w-full py-3 px-4 rounded-xl font-black text-xs text-white cursor-pointer transition-all mt-1"
